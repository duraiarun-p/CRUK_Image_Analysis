...
# base_dir='/mnt/local_share/TMA/FS-FLIM/raw/Tumour_2B/'
...
base_index=[]
file_extension_type='_TX.mat'
for core_file in base_dir_all:
    core_stitched_TX_file='/mnt/local_share/TMA/FS-FLIM/raw/Tumour_2B/'+core_file+'/Mat_output2/core_stitched_TX.mat'
    ind = base_dir_all.index(core_file)
    if os.path.isfile(core_stitched_TX_file):
        base_index.append(1)
        # del base_dir_all[ind]
    else:
        base_index.append(0)
base_index=np.array(base_index)
base_index_0=np.array(np.argwhere(base_index==0))
# c=0
# for index,ind in zip(base_index,base_dir_all):
#     if index==0:
#         print(base_dir_all.index(ind))

# for del_ind in base_index_0:    
#     del base_dir_all[del_ind]
# del base_dir_all[base_index_0]

base_dir_all_F = [base_dir_all[i] for i in np.arange(68,len(base_dir_all)) if i not in base_index_0]
#%%
for core_file in base_dir_all_F:
    base_dir='/mnt/local_share/TMA/FS-FLIM/raw/Tumour_2B/'+core_file+'/Mat_output2'
    core_mat_cont_file=base_dir+'/core_stitched_TX.mat'
    print(core_mat_cont_file)
    ...
    stitch_intensity=np.transpose(stitch_intensity,axes=[1,0])
    stitch_intensity_cube=np.transpose(stitch_intensity_cube,axes=[2,1,0])
    stitch_flt_cube=np.transpose(stitch_flt_cube,axes=[2,1,0])
    ...
    for page in range(flt_cube_shape[2]):# Changed for the new datatype
    ...
    # savemat(f"{base_dir}/core_stitched_masked_T.mat", mdic)
    matfile_name=base_dir+'/core_stitched_masked_TX.mat'
    # f = h5py.File(matfile_name, mode='w')
    hdf5storage.savemat(matfile_name, mdic,format='7.3',oned_as='row',store_python_metadata=True)
    # hdf5storage.write(mdic,'.',matfile_name,matlab_compatible=True)
    print(matfile_name)
    # print(base_dir_all_F.index(core_file))