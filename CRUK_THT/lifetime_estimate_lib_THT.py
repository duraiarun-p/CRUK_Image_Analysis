...
def life_time_est_cf_exp(bin_resp,time_line,bin_size,time_indices,time_index):
    ...
    # tau, r_squared=flt_est_cf_exp(time_line_selected, bin_resp_selected)
    tau, r_squared=flt_est_cf_exp(time_line, bin_resp)
    # tau=abs(popt[1])
...
def prepare_data_list_log(frame_size,bin_array,bin_size,time_index,time_indices,time_line):
    ...
    bin_Len=len(bin_list) # total number of pixel elements
    return bin_Len,bin_list,time_list,bin_index_list,bin_log_list_partial,time_list_partial
