...
%% Parameters
nopeaks=3;
no_of_spectral_channels=5;
%% Fit analysis
currentFolder = pwd;
filePath = uigetdir; 
[Int_Prof,bins_cell,F_sp_s,F_sp,lambdas,F_sp_lambdas]=auto_spectral_selection(filePath,currentFolder,nopeaks,no_of_spectral_channels);

% lambdas=1:320;
%%
tic;
[allIntensityImages,lifetimeImageData,lifetimeAlphaData]=Analysis_LMfitting(filePath,currentFolder,bins_cell,lambdas,[12,15]); % LS fitting
time_LS=toc;
tic;
[allIntensityImages2,lifetimeImageData2,lifetimeAlphaData2]=Analysis_EXPfitting(filePath,currentFolder,bins_cell,lambdas,[12,15]); % Partial Exp fitting
time_PEXP=toc;
% tic;
% [allIntensityImages3,lifetimeImageData3,lifetimeAlphaData3]=Analysis_EXPfitting(filePath,currentFolder,bins_cell,lambdas,[1,1]); % Exp fitting
% time_EXP=toc;
%% 
...
[SSIM_F12,IMDIFF_F12]=compare_lfs_exp(allIntensityImages,lifetimeAlphaData,lifetimeImageData,lifetimeImageData2,10,'LS vs Partial Exp');
[SSIM_F13,IMDIFF_F13]=compare_lfs_exp(allIntensityImages,lifetimeAlphaData,lifetimeImageData,lifetimeImageData3,20,'LS vs Full Exp');
...
% Spectral selection
function [Int_Prof,bins_cell,F_sp_s,F_sp,lambdas,F_sp_lambdas]=auto_spectral_selection(filePath,currentFolder,nopeaks,numberoflambdas)
[Int_Prof,bins_cell]=myread_file(filePath,currentFolder,numberoflambdas);
...
% Spectral Analysis
function [Int_Prof,bins_cell]=myread_file(filePath,currentFolder,no_of_spectral_channels)
...
%         bins_cell{file_ind,1}=bins_array_3;
...
%Sub function required for spectral analysis
...
function [allIntensityImages,lifetimeImageData,lifetimeAlphaData]=Analysis_LMfitting(filePath,currentFolder,bins_cell,lambdas,binToFit)
% Temporal resolution
n_bins = 16;
...
function [allIntensityImages,lifetimeImageData,lifetimeAlphaData]=Analysis_EXPfitting(filePath,currentFolder,bins_cell,lambdas,binToFit)
% Temporal resolution
n_bins = 16;
...
%         bins_array_3 = permute(bins_array_3, [4 3 1 2]); % This is the place that check for array dimension and changes if needed
        ...
        bins_array_3=bins_cell{file_ind,1};
        bins_array_3=bins_array_3(lambdas,:,:,:);
